...
def saveToCSV(name: str, dataList: List):
    if len(dataList) > 0:
        if (list(vars(dataList[0]).keys())[0] == "id"):
            df = pd.DataFrame([[data.id, data.angle, data.x, data.y, 1 if data.cluster.name == "Red" else 2 if data.cluster.name == "Blue" else 0 ] for data in dataList ])
        else: 
            df = pd.DataFrame([[data.first.id, data.second.id] for data in dataList ])
        df.to_csv(name + '.csv', header=list(vars(dataList[0]).keys()), index=False)
    else: return 
...
def loadFromCSV(verticesName: str):
    ...
    if (header == ['id', 'angle', 'x', 'y', 'cluster']):
        for line in newData.values:
            elements.append(Point(id=line[0], angle=line[1], x=line[2], y=line[3], cluster=Cluster(line[4])))      
        global vertices 
        vertices = elements     
        return elements
    elif (header == ['first', 'second']):
        for line in newData.values:
            elements.append(Connection(first=[vertex for vertex in vertices if vertex.id == int(line[0])][0], second=[vertex for vertex in vertices if vertex.id == int(line[1])][0]))
        return elements
...
def visualizeRandomGeometricGraph(vertices: List[Point], edges: List[Connection], pictureName):
    ...
    for edge in edges:
        plt.plot([edge.first.x, edge.second.x], [edge.first.y, edge.second.y], c='green')
